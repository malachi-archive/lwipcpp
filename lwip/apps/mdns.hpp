...
class mDNSResponder
{
    ...
    {
        inline static_constructor()
        {
#if MDNS_INIT_STATIC_CONSTRUCTOR
            mdns_resp_init();
#endif
        }

        static void crash_if_no_cpp();
    };
...
#if MDNS_INIT_STATIC_CONSTRUCTOR
        static_constructor::crash_if_no_cpp();
#else
#warning "Be careful, mdns_resp_init called for every mDNSResponder"
        mdns_resp_init();
#endif
...
#define MDNS_STATIC_CONSTRUCTOR_INSTANCE() \
namespace lwip { namespace mdns { \
\
mDNSResponder::static_constructor mDNSResponder::_sc; \
\
void mDNSResponder::static_constructor::crash_if_no_cpp() {} \
\
}}
